...
> **플랫폼**: 미드저니 웹 UI (midjourney.com) 전용. 디스코드 봇의 `/imagine` 명령어는 사용하지 않습니다.
...
- "무드보드", "스타일 크리에이터", "개인화 설정", "퍼스널라이즈", "내 스타일 만들기"
...
| **스타일 제어(Style Control)** | Personalize/Moodboard/Style Creator 사용 여부 | 무드보드 코드, 개인화 프로필, 커스텀 sref |

명시되지 않은 요소는 용도와 맥락에서 추론합니다.
스타일 제어 관련 요청이 있으면 `references/personalize-moodboard-style-creator.md`를 참조합니다.
...
웹 UI (midjourney.com)의 Imagine 바에 바로 붙여넣을 수 있도록 프롬프트 텍스트만 출력합니다.
...
### 출력 규칙
1. 프롬프트는 항상 코드 블록 안에 텍스트만 표시 (웹 UI Imagine 바에 바로 붙여넣기용 — `/imagine prompt:` 접두사 사용 금지)
...
7. 프로젝트 일관성이 중요한 요청에는 Moodboard/Style Creator/Personalize 활용을 선택적으로 제안 (아래 형식)

### 스타일 제어 제안 (선택적 출력)

프로젝트 일관성, 브랜드 작업, 반복 생성 등 스타일 통일이 필요한 맥락에서 프롬프트와 함께 제안합니다:

```
### 스타일 일관성 팁
- **반복 사용할 미학이라면**: Style Creator(`.`으로 시작)로 커스텀 `--sref` 코드를 만들어 재사용하세요
- **레퍼런스 이미지가 있다면**: Moodboard에 이미지를 모아 `--profile [code]`로 적용하세요
- **개인 취향을 자동 반영하려면**: Personalize 버튼을 ON으로 설정하세요
```
...
| 사용자 요청 | 추천 방식 | 웹 UI 섹션 |
|------------|----------|-----------|
| "이 캐릭터로 다른 포즈" | --oref + --sref | Omni Reference + Style References |
| "이 캐릭터를 정확히 이 포즈로" | 포즈 사진(Image Prompts) + --oref | Image Prompts + Omni Reference |
| "이 캐릭터 의상만 변경" | --oref (낮은 --ow) + --sref | Omni Reference + Style References |
| "이 그림체로 다른 캐릭터" | --sref만 사용 | Style References만 |
| "이 이미지 분위기 그대로 재현" | Image Prompts | Image Prompts |
| "이 이미지를 영상으로" | Start Frame + --motion | Start Frame |
...
4. **--stylize 낮게**: 50~150으로 설정해야 --oref/--sref 효과가 강해짐

### 주의사항

- **Editor(Edit 버튼) 모드에서는 포즈 변경 불가** → 반드시 Create 페이지 사용
- **Image Prompts에 넣으면 포즈 고정됨** → Omni Reference에 넣어야 포즈 변경 가능
- **--oref만 쓰면 그림체 달라짐** → --sref를 같은 이미지로 함께 사용 (Shift+클릭)
- **Omni Reference는 Draft/Fast 모드 비호환** → Standard 또는 Turbo 모드 사용
...
Editor는 `--ar`, `--stylize`, `--oref`, `--sref` 등 **파라미터를 사용할 수 없으며**, 텍스트 프롬프트만으로 결과를 제어한다.
...
- 최초 ~200장 이미지 평가(약 5분)가 V7 사용의 전제조건
- **프로필 3종**: Global(범용 취향), Ranked(특정 스타일 전용), Moodboard(이미지 큐레이션)
- 웹 UI: Imagine 바 옆 **Personalize 버튼**으로 ON/OFF 토글, 드롭다운으로 프로필 전환
- `--p` 파라미터로 수동 적용, `--p code1 code2`로 다중 프로필 블렌딩 가능
- **프롬프트 팁**: Personalize ON이면 스타일 키워드를 줄이고 "무엇을" 묘사에 집중

### Moodboards (무드보드)
- 이미지를 직접 큐레이션하여 미드저니를 학습시키는 기능 (웹 전용)
- `midjourney.com/personalize` → "Create Moodboard" → 이미지 업로드/URL/갤러리에서 선택 (최대 100장)
- **2가지 코드**: M-code(동적, 무드보드에 실시간 연동) vs Snapshot code(정적, 영구 유지)
- `--profile [snapshot code]`로 프롬프트에 적용
- Moodboard끼리 또는 `--sref`와 블렌딩 가능 (가중치는 `--sref`만 지원)
- **최적 용도**: 브랜드 일관성, 클라이언트별 미학, 팀 공유

### Style Creator (스타일 크리에이터)
- 시각적 선호도 선택으로 **커스텀 `--sref` 코드**를 생성하는 웹 전용 도구
- 좌측 메뉴 → Style Creator → 프롬프트 입력(`.` 추천) → 그리드에서 선호 이미지 선택 → 반복 리파인
- **Dot method**: `.`만 입력하면 주제 무관한 범용 스타일 코드 생성 (이식성 최대)
- **5~15 라운드**가 실용적 최적 구간 (1~5 탐색, 5~10 안정화, 10~15 정교화)
- 생성된 코드: `--sref [숫자코드]`로 사용, `--sw`로 강도 조절
- **반드시 코드 메모** — 숫자로만 되어 있어 나중에 검색 불가

### 3가지 기능 조합 활용

| 워크플로우 | 추천 조합 |
|-----------|----------|
| 개인 자유 탐색 | Personalization ON (기본) |
| 특정 미학 프로젝트 | Ranked Profile + Style Creator 코드 |
| 클라이언트/브랜드 | Moodboard(Snapshot) + --sref |
| 팀 협업 | Moodboard Snapshot code 공유 |
| 기존 캐릭터 + 미학 | Moodboard + --oref + --sref |

상세 가이드: `references/personalize-moodboard-style-creator.md` 참조

### Style Version (--sv)
- `--sref`와 함께 사용하여 스타일 참조 해석 방식 제어
- `--sv 1`~`--sv 6` (기본: 6) — 낮을수록 추상적, 높을수록 충실한 스타일 재현
- Style Creator(2025.06.16 이후) 코드는 자동 `--sv 6`, 레거시 코드는 수동 `--sv 4` 필요
...
| `references/personalize-moodboard-style-creator.md` | Personalize, Moodboards, Style Creator 활용 가이드 |
| `references/video-generation-guide.md` | 비디오 생성 요청 시 |
