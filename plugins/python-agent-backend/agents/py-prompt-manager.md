---
name: py-prompt-manager
description: YAML 기반 프롬프트 버전 관리 전문가. 버전 관리, A/B 테스트, 변수 치환, 롤백 자동화.
model: sonnet
...
YAML 기반 프롬프트를 관리하고 버전 관리하는 전문가.
...
class ABTestManager:
    ...
    def end_test(self, test_name: str) -> dict:
        ...
        if test_name in self.active_tests:
            self.active_tests[test_name].end_time = datetime.now()
            results["end_time"] = self.active_tests[test_name].end_time.isoformat()
...
# 버전 변경
python -c "from prompt_manager import PromptVersionManager; m = PromptVersionManager(); m.set_version('orchestrator/supervisor', 'v2')"

# 프롬프트 테스트
pytest tests/prompts/ -v

# YAML 유효성 검사
python -c "import yaml; yaml.safe_load(open('prompts/orchestrator/supervisor.yaml'))"
```
