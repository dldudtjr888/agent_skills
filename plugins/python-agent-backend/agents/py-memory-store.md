---
name: py-memory-store
description: 3-tier 메모리 시스템 구축 전문가. Cache/Session/Long-term 메모리, Context Assembly, Memory Promotion.
model: opus
...
class MemoryType(str, Enum):
    """메모리 유형"""
    CONVERSATION = "conversation"  # 대화 내역
    KNOWLEDGE = "knowledge"        # 지식/정보
    TASK = "task"                  # 작업/할 일
    PREFERENCE = "preference"      # 사용자 선호
...
class GetContextResult(TypedDict):
    ...
    count: int
    sources: dict[str, bool]  # {tier: included}
    error: Optional[str]
```
...
class MemoryManager:
    ...
    async def save_important(
        ...
        entry = MemoryEntry(
            ...
            accessed_at=datetime.now(),
            access_count=0,
            embedding=None,  # LongTermStore에서 생성
        )
...
class AgentMemoryAdapter:
    ...
    async def prepare_input(
        ...
        relevant_context = await self.manager.get_context(
            ...
            include_cache=True,
            include_session=False,  # 이미 최근 메시지로 포함
    ...
    async def _trigger_promotion(self) -> None:
        """중요 메모리 장기 저장소로 승격"""
        if not self.manager.session or not self.manager.longterm:
            return

        # 높은 우선순위 또는 자주 접근된 메모리 승격
        # (구현 세부사항은 프로젝트에 따라)
        pass
    ...
    async def save_important(
        ...
        """중요 정보 직접 장기 저장"""
        return await self.manager.save_important(
            content=content,
            user_id=self.user_id,
            memory_type=memory_type,
...
# 임베딩 테스트
python -c "from memory import VectorStore; ..."

# 타입 체크
ty check navis/memory/
```
