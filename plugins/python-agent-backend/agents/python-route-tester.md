...
async def performance_test():
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        for method, path, kwargs in endpoints:
            result = await measure_response_time(client, method, path, **kwargs)
            print(f"{method} {path}:")
...
class APIUser(HttpUser):
    ...
    def create_item(self):
        self.client.post(
            "/items/",
            json={"name": "Load Test Item", "price": 100},
            headers={"Authorization": f"Bearer {self.token}"},
        )
        ...
        async with semaphore:
            try:
                ...
                if response.status_code == 200:
                    ...
                    results["failure"] += 1
                    results["errors"].append(f"Request {i}: {response.status_code}")
            except Exception as e:
                results["failure"] += 1
                results["errors"].append(f"Request {i}: {e}")

    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        tasks = [make_request(client, i) for i in range(total)]
        await asyncio.gather(*tasks)

    print(f"Success: {results['success']}/{total}")
    print(f"Failure: {results['failure']}/{total}")
    if results["errors"]:
        print(f"Errors: {results['errors'][:5]}...")  # 처음 5개만

# 사용
asyncio.run(concurrent_requests_test("/health", concurrent=50, total=500))
```
...
async def test_race_condition():
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        response = await client.post(
            "/items/",
            json={"name": "Race Test", "price": 100},
            headers=headers,
        )
        item_id = response.json()["id"]

        # 2. 동시에 같은 아이템 수정
        async def update_item(new_price: int):
            return await client.put(
                f"/items/{item_id}",
                json={"name": "Race Test", "price": new_price},
                headers=headers,
            )

        # 10개 동시 수정 요청
        tasks = [update_item(i * 10) for i in range(10)]
        responses = await asyncio.gather(*tasks, return_exceptions=True)

        # 결과 확인
        success = sum(1 for r in responses if not isinstance(r, Exception) and r.status_code == 200)
        print(f"Concurrent updates: {success}/10 succeeded")

        # 최종 상태 확인
        final = await client.get(f"/items/{item_id}", headers=headers)
        print(f"Final price: {final.json()['price']}")
```
...
@pytest.mark.asyncio
async def test_concurrent_create(client, auth_headers):
    """동시 생성 테스트"""
    ...
    # 10개 동시 생성
    tasks = [create_item(f"Concurrent Item {i}") for i in range(10)]
